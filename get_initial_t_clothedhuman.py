...
from utils.BNI import BNI
from utils.BNI_utils import save_normal_tensor
...
from utils.normal_fusion import canonicalize_normal_map, merge_normal_images, merge_normal_images_cover
...
if __name__ == "__main__":
    ...
    ###### gtnormal2tnormal ###### 还没写
    ### 第一种方法来自selfrecon：是通过deformer变过来，但是deformer此时还未知（如果把这个代码放在中间步骤是已知的）
    gtnormals = datas['normal'].to(device)
    gtnormals = ((cameras.R[0] @ torch.tensor([[-1., 0., 0.], [0., 1., 0.], [0., 0., -1.]],
                                                    device=device)) @ gtnormals.view(-1, 3, 1)).view(-1, 3)
    gtnorms_lpips = gtnormals.norm(dim=1, keepdim=True)
    valid_mask_lpips = (gtnorms_lpips > 0.0001)[..., 0]
    gtnormals[valid_mask_lpips] = gtnormals[valid_mask_lpips] / gtnorms_lpips[valid_mask_lpips]
    ds_lpips = self.deformer(self.TmpPs_lpips, defconds, correct_pose, self.batch_inds_lpips, ratio=ratio)
    grad_d_p_lpips = utils.compute_Jacobian(self.TmpPs_lpips, ds_lpips, True, True)  # J
    # 通过j变换到t-pose normal
    gtnormals_lpips = (grad_d_p_lpips.transpose(-2, -1) @ gtnormals_lpips.view(-1, 3, 1)).view(-1, 3)
    print('gt normal transform to t-pose normal')

    ### 第二种方法来自avatarcap：
    vertices, faces, normals = recon_util.recon_mesh(occ_volume, cfg['testing']['vol_res'], bounds,
                                                     iso_value=config.iso_value)   # cannonical
    ...
    # bmin bmax
    cano_smpl_center = 0.5 * (
                smpl_util.cano_smpl_vertices.max(0)[0] + smpl_util.cano_smpl_vertices.min(0)[0]).cpu().numpy()
    ...
    live_vertices, vert_mats = smpl_util.skinning(vertices_gpu[None, ...], lbs, items['cano2live_jnt_mats'], True)
    front_image_normal, back_image_normal = canonicalize_normal_map(position_renderer, normal_renderer, vertices,
                                                                    live_vertices, faces, inferred_normal, vert_mats,
                                                                    mv=items['w2c_RT'][0].cpu().numpy(), fx=cam['fx'],
                                                                    fy=cam['fy'], cx=cam['cx'], cy=cam['cy'],
                                                                    cano_smpl_center=cano_smpl_center_for_render)

    ###### gtmask2tmask ###### 还没写

    print('gt mask transform to t-pose mask')
    ...
    for data in pbar:
        ...
        if osp.exists(smpl_path):
            ...
            for i in loop_smpl:
                ...
                with torch.no_grad():   # image_based_normal
                    in_tensor["normal_F"], in_tensor["normal_B"] = normal_net.netG(in_tensor)
                ...
                bg_value = in_tensor["T_normal_F"][0, 0, 0, 0]   # 背景值
                smpl_arr_fake = torch.cat(    # 根据背景值判断该点是否为背景，不等于返回true。得到smpl的区域。
                    [
                        in_tensor["T_normal_F"][:, 0].ne(bg_value).float(),   # ne()逐元素比较，不等于返回true
                ...
                if (i == args.loop_smpl - 1) and (not args.novis):
                    ...
                    per_loop_lst.extend(
                        [
                            ...
                            in_tensor["normal_F"],
                            diff_S[:, :, :512].unsqueeze(1).repeat(1, 3, 1, 1),   # smpl与cloth_human的轮廓差
                        ]
                    )
                    per_loop_lst.extend(
                        [
        ...
        # 通过预设好的两个摄像机和MeshRasterizer获得在这两个相机下的深度
        # depth 有问题
        ...
        for idx in range(N_body):
            ...
            # save normals（smpl&img）, depths（smpl） and masks（smpl&img）
            BNI_dict = save_normal_tensor(
                in_tensor,
                idx,
                osp.join(args.out_dir, cfg.name, f"BNI/{data['name']}_{idx}"),
                cfg.bni.thickness,   # 0.00
            )
