...
with open(osp.join(root,'camera.pkl'),'rb') as ff:
	...
	trans=cam_data['camera_t']   # 啥也不是 写进pkl的时候就是0
	rt=cam_data['camera_rt']
	extrinsic = cam_data['camera_e']
	assert(np.linalg.norm(rt)<0.0001) # The cameras of snapshot dataset seems no rotation and translation 相机固定的意思
	H=cam_data['height']
	W=cam_data['width']
	...
	np.savez(osp.join(save_root,'camera.npz'),fx=fx,fy=fy,cx=cx,cy=cy,quat=quat,T=T,extrinsic=extrinsic)
